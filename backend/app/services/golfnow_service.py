...
async def fetch_golfnow_times(
    ...
    logger.info(f"[GolfNow] Fetching {course['name']} | facility={facility_id} | date={date_str}")
...
async def normalize_and_store(
    ...
    if ids_to_remove:
        logger.info(
            f"[GolfNow] Removing {len(ids_to_remove)} stale tee times "
    ...
    if upsert_rows:
        ...
        logger.info(
            f"[GolfNow] Upserted {len(upsert_rows)} tee times "
            f"for {course['name']} on {date_str}"
        )
    else:
        logger.info(f"[GolfNow] No tee times found for {course['name']} on {date_str}")
...
async def run_golfnow_scan() -> None:
    ...
    logger.info(f"[GolfNow] Starting scan at {start_time.isoformat()}")

    targets = await get_active_golfnow_targets()

    if not targets:
        logger.info("[GolfNow] No active alerts found. Skipping scan.")
        return

    logger.info(f"[GolfNow] Found {len(targets)} unique (course, date) pairs to scan.")
